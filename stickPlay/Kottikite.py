...
ko = gausian_pulse(k, 0.04, 55/200, 1)
ti = gausian_pulse(k, 0.02, 100/200, 0.5)
ki = gausian_pulse(k, 0.02, 120/200, 0.6)
te = gausian_pulse(k, 0.02, 150/200, 0.9)
...
def keyloop():
    ...
    while LoopFlag:
        if clients is not None:  # クライアントが接続されている場合のみ送信
            try:
                if dot < 0.3: 
                    if i < len(kottikite) - 1:
                        i += 1
                    else: 
                        i = 0                   
                    value = int(255 * kottikite[i])
                    ...
                    if j < len(sin_wave_unipolar) - 1:
                        ...
                        j = 0        
                    # red_value = int(255 * sin_wave_unipolar[j])
                    # green_value = int(128 * sin_wave_unipolar[j])  # 赤の半分
                    # message = f'LED 0,{red_value},{green_value},0'
