...
normal_sin_wave = np.sin(t)  # -1～1のsin波
normal_sin_wave_unipolar = (normal_sin_wave + 1) / 2
...
def keyloop():
    ...
    while LoopFlag:
        if clients is not None:  # クライアントが接続されている場合のみ送信
            try:
                if dot < 0.3: 
                    if i < len(normal_sin_wave_unipolar) - 1:
                        i += 1
                    else: 
                        i = 0                   
                    value = int(255 * normal_sin_wave_unipolar[i])
